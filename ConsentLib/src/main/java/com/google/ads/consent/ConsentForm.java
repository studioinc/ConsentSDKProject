...
/**
 * A Google rendered form for collecting consentManager from a user.
 */
public class ConsentForm {
        ...
        webView.setWebViewClient(
            new WebViewClient() {
                ...
                private boolean isConsentFormUrl(String url) {
                    return !TextUtils.isEmpty(url) && url.startsWith("consentManager://");
                }
    ...
    private void updateDialogContent(WebView webView) {
        ...
        formInfo.put("is_request_in_eea_or_unknown", ConsentInformation.getInstance(context).isRequestLocationInEeaOrUnknown());
        ...
        String javascriptCommand = createJavascriptCommand("setUpConsentDialog", argumentsJSON);
        webView.loadUrl(javascriptCommand);
    }

    public void load() {
        if (this.loadState == LoadState.LOADING) {
            listener.onConsentFormError("Cannot simultaneously load multiple consentManager forms.");
            return;
        }
        ...
        this.webView.loadUrl("file:///android_asset/form.html");
    }
    ...
    public void show() {
        if (this.loadState != LoadState.LOADED) {
            listener.onConsentFormError("ConsentManager form is not ready to be displayed.");
            return;
        }

        if (ConsentInformation.getInstance(context).isTaggedForUnderAgeOfConsent()) {
            listener.onConsentFormError("Error: tagged for under age of consentManager");
            return;
        }
        ...
        if (!this.dialog.isShowing()) {
            listener.onConsentFormError("ConsentManager form could not be displayed.");
        }
    }
